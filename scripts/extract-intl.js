...
const { appLocales, DEFAULT_LOCALE } = require('../src/i18n/constants');
...
for (const locale of appLocales) {
  ...
  const translationFileName = `src/i18n/translations/${locale}.json`;
  try {
...
const extractFromFile = async filename => {
  try {
    ...
    const output = await transform(code, { filename, presets: ['react-app'], plugins: ['react-intl'] });
...
memoryTask.then(files => {
  ...
  extractTask.then(() => {
    ...
    mkdir('-p', 'src/i18n/translations'); // eslint-disable-line

    let localeTaskDone;
    let translationFileName;

    for (const locale of appLocales) {
      translationFileName = `src/i18n/translations/${locale}.json`;
